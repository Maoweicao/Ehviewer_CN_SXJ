...
import com.hippo.ehviewer.widget.RatingRangeView;
...
public class DownloadsScene extends ToolbarScene
    ...
    private static final String KEY_RESUME_HISTORY_CHECKED = "resume_history_checked";
...
//    private final int[] perPageCountChoices = {1, 2, 3, 4, 5};
    ...
    private boolean mResumeFromHistoryChecked = false;
...
//        mPaginationIndicator.setTotalCount();
    ...
    private void onRestore(@NonNull Bundle savedInstanceState) {
        ...
        mResumeFromHistoryChecked = savedInstanceState.getBoolean(KEY_RESUME_HISTORY_CHECKED, false);
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        ...
        outState.putBoolean(KEY_RESUME_HISTORY_CHECKED, mResumeFromHistoryChecked);
    }
    ...
    private boolean isQuickFilterId(int id) {
        switch (id) {
            ...
            case R.id.quick_filter_ai_generated:
                return true;
            default:
                return false;
        }
...
//        mRecyclerView.setOnGenericMotionListener(this::onGenericMotion);
    ...
    public void onResume() {
        ...
        // 自动检查是否有可恢复的下载历史
        checkAutoResumeFromHistory();
    }

    /**
     * 如果开启了"自动从下载历史恢复"设置，检查是否有可恢复的下载并提示用户
     */
    private void checkAutoResumeFromHistory() {
        if (!Settings.getAutoResumeFromHistory()) return;
        if (mResumeFromHistoryChecked) return;
        mResumeFromHistoryChecked = true;

        // 异步检查避免阻塞UI
        new Thread(() -> {
            try {
                java.util.List<com.hippo.ehviewer.dao.DownloadHistory> allHistory = EhDB.getAllDownloadHistory();
                if (allHistory == null || allHistory.isEmpty()) return;

                int resumableCount = 0;
                for (com.hippo.ehviewer.dao.DownloadHistory history : allHistory) {
                    // 仅统计当前有效（用户未主动删除/合并）且不在下载列表中的历史
                    if (history.getDeletionType() == com.hippo.ehviewer.dao.DownloadHistory.DELETION_NONE
                            && !mDownloadManager.containDownloadInfo(history.getGid())) {
                        resumableCount++;
                    }
                }
                final int count = resumableCount;
                ...
                if (count > 0) {
                    ...
                    activity.runOnUiThread(() -> {
                        ...
                        new AlertDialog.Builder(requireContext())
                            .setTitle(R.string.resume_from_history_title)
                            .setMessage(getString(R.string.resume_from_history_message, count))
                            .setPositiveButton(android.R.string.ok, (d, w) -> {
                                ...
                                if (taskManager.getTaskStatusManager().getActiveUniqueNonDownloadTask() != null) {
                                    Toast.makeText(getContext(), R.string.background_task_unique_running, Toast.LENGTH_SHORT).show();
                                    return;
                                }
                                ResumeFromHistoryTask task = new ResumeFromHistoryTask(requireContext());
                                taskManager.submitBackgroundTask(task);
                                Toast.makeText(getContext(), R.string.settings_download_resume_started, Toast.LENGTH_SHORT).show();
                            })
                ...
                Log.w(TAG, "checkAutoResumeFromHistory failed", e);
            }
        }).start();
    }
...
//                return true;
    ...
    private void gotoSearch(Context context) {
        ...
        // 恒定功能：左键显示搜索历史，右键触发搜索
        mSearchBar.setLeftDrawable(DrawableManager.getVectorDrawable(context, R.drawable.v_history_black_x24));
        ...
        RatingRangeView ratingRangeView = linearLayout.findViewById(R.id.rating_range_view);
        CheckBox pagesRangeCheckbox = linearLayout.findViewById(R.id.pages_range_checkbox);
        EditText pagesFromInput = linearLayout.findViewById(R.id.pages_from_input);
        EditText pagesToInput = linearLayout.findViewById(R.id.pages_to_input);
        EditText timeFromInput = linearLayout.findViewById(R.id.time_from_input);
        EditText timeToInput = linearLayout.findViewById(R.id.time_to_input);
        EditText sizeFromInput = linearLayout.findViewById(R.id.size_from_input);
        EditText sizeToInput = linearLayout.findViewById(R.id.size_to_input);
        CheckBox duplicateOnlyCheckbox = linearLayout.findViewById(R.id.duplicate_only_checkbox);
        ...
        // Status select all/none buttons
        ...
        sortNames.add(context.getString(R.string.sort_by_remaining_pages_asc));
        sortIds.add(R.id.sort_by_remaining_pages_asc);
        sortNames.add(context.getString(R.string.sort_by_remaining_pages_desc));
        sortIds.add(R.id.sort_by_remaining_pages_desc);
        ...
        // Date picker buttons for time inputs
        Button pickTimeFromBtn = new Button(context);
        Button pickTimeToBtn = new Button(context);
        ...
        if (mAdvancedFilterActive) {
            ...
            // Restore page range
            if (mFilterPageFrom != null) {
                pagesFromInput.setText(String.valueOf(mFilterPageFrom));
                pagesRangeCheckbox.setChecked(true);
            }
            if (mFilterPageTo != null) {
                pagesToInput.setText(String.valueOf(mFilterPageTo));
            }
            // Restore duplicate only
            duplicateOnlyCheckbox.setChecked(mFilterDuplicateOnly);
            // Restore rating range
            if (ratingRangeView != null) {
                ratingRangeView.setRangeEnabled(true);
                ratingRangeView.setRatingRange(mFilterRatingFrom, mFilterRatingTo);
            }
        }
        ...
        resetButton.setOnClickListener(v -> {
            ...
            // Reset rating range
            if (ratingRangeView != null) {
                ratingRangeView.reset();
            }
            // Reset page range
            pagesRangeCheckbox.setChecked(false);
            pagesFromInput.setText("");
            pagesToInput.setText("");
            // Reset time range
        ...
        searchButton.setOnClickListener(v -> {
            ...
            float ratingFrom = 0f;
            float ratingTo = 5f;
            if (ratingRangeView != null && ratingRangeView.isEnabled()) {
                ratingFrom = ratingRangeView.getRatingFrom();
                ratingTo = ratingRangeView.getRatingTo();
            }

            // Get page range
            Long pageFrom = null;
            Long pageTo = null;
            if (pagesRangeCheckbox.isChecked()) {
                String fromText = pagesFromInput.getText().toString().trim();
                String toText = pagesToInput.getText().toString().trim();
                if (!fromText.isEmpty()) {
                    try {
                        pageFrom = Long.parseLong(fromText);
                    } catch (NumberFormatException ignored) {}
                }
                if (!toText.isEmpty()) {
                    try {
                        pageTo = Long.parseLong(toText);
                    } catch (NumberFormatException ignored) {}
                }
            }
            ...
            // Get duplicate only
            boolean duplicateOnly = duplicateOnlyCheckbox.isChecked();

            // Stash the search/filter state so it survives rotation and list refreshes
            mSearchActive = !searchKey.isEmpty();
            mSearchOption = searchOption;
            mSearchCategoryIds = categories;
            mSearchSortId = sortId;

            // Stash advanced filter state
            ...
            mFilterRatingFrom = ratingFrom;
            mFilterRatingTo = ratingTo;
            ...
            executor.setDownloadSearchingListener(this);
            cancelCurrentExecutor();
            executor.executeFilterAndSort(categories, selectedStatusIds, sortId,
                    timeFrom, timeTo, sizeFrom, sizeTo,
                    pageFrom, pageTo, duplicateOnly, ratingFrom, ratingTo);
            mCurrentExecutor = executor;
            searching = true;
        });
    }
...
//            startActivity(intent);
    ...
    public boolean onItemLongClick(EasyRecyclerView parent, View view, int position, long id) {
        ...
        MergedGalleryChecker.check(context, di, target ->
                showLongClickDialog(parent, view, position, id, di, favourited, isRelayDownload, target));
        return true;
    }

    private void showLongClickDialog(EasyRecyclerView parent, View view, int position, long id,
                                     DownloadInfo di, boolean favourited, boolean isRelayDownload,
        ...
        items.add(context.getString(isRelayDownload ? R.string.relay_cancel_download : R.string.relay_send_to_device));
        icons.add(R.drawable.v_send_dark_x24);
        items.add(context.getString(R.string.delete));
        icons.add(R.drawable.v_delete_x24);
        final int indexJump;
        ...
        final int indexFavorites = 1;
        final int indexRelay = 2;
        final int indexDelete = 3;
        ...
        new AlertDialog.Builder(getDialogContext())
                ...
                .setAdapter(new SelectItemWithIconAdapter(context, itemArray, iconArray), (dialog, which) -> {
                    ...
                    } else if (which == indexRelay) { // Relay Download / Cancel Relay
                        if (isRelayDownload) {
                            // 取消接力下载，恢复为本机下载
                            if (mDownloadManager != null) {
                    ...
                    } else if (which == indexDelete) { // Delete
...
//            mSpiderInfoMap.put(info.gid,getSpiderInfo(info));
    ...
    protected void startSearching() {
        ...
        DownloadListInfosExecutor executor = new DownloadListInfosExecutor(mBackList, searchKey, getContext());
